#!/usr/bin/env bash
# hbvless deploy, round 2: fresh LE cert + nginx mux + docker container.
set -euo pipefail
...
if [ ! -f /etc/rustdesk-vless/hbvless.env ]; then
...
echo '==> [2] webroot http check'
echo probe-ok > /www/wwwroot/nas.your-domain.example/.well-known/acme-challenge/probe.txt
chown www:www /www/wwwroot/nas.your-domain.example/.well-known/acme-challenge/probe.txt
sleep 1
curl -s --max-time 10 "http://$DOMAIN/.well-known/acme-challenge/probe.txt" && echo ' <- webroot OK'
rm -f /www/wwwroot/nas.your-domain.example/.well-known/acme-challenge/probe.txt

echo '==> [3] fresh LE cert'
cd ~/.acme.sh
for f in nas.your-domain.example nas.your-domain.example_ecc nas.your-domain.example.conf nas.your-domain.example.csr \
         nas.your-domain.example.csr.conf nas.your-domain.example.key nas.your-domain.example.cer nas.your-domain.example.ca.cer; do
  [ -e "$f" ] && mv "$f" "$f.vlessbak.$TS"
done
~/.acme.sh/acme.sh --issue -d "$DOMAIN" --webroot /www/wwwroot/nas.your-domain.example \
    --server letsencrypt --keylength ec-256
openssl x509 -in ~/.acme.sh/"${DOMAIN}"_ecc/fullchain.cer -noout -subject -dates
~/.acme.sh/acme.sh --install-cert -d "$DOMAIN" --ecc \
    --fullchain-file /etc/rustdesk-vless/nas.fullchain.pem \
    --key-file /etc/rustdesk-vless/nas.key \
    --reloadcmd 'docker restart hbvless 2>/dev/null || true'
