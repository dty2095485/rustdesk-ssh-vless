        ...
        Container(
          child: Column(
            ...
            children: [
              ...
              if (!hideProxy)
                futureBuilder(
                  future: bind.mainGetVless(),
                  hasData: (vlessValues) {
                    final enabled = vlessValues is List &&
                        ...
                        onChanged: locked
                            ...
                            : (value) async {
                                if (value) {
                                  ...
                                  // OFF: keep the values cached and switch to
                                  // the direct ID/relay connection mode.
                                  if (vlessValues is List &&
                                      vlessValues.length >= 4) {
                                    _vlessCache =
              ...
              if (!hideProxy)
                futureBuilder(
                  future: bind.mainGetSsh(),
                  hasData: (sshValues) {
                    final enabled = sshValues is List &&
                        sshValues.length >= 6 &&
                        sshValues[5] == 'Y';
                    return listTile(
...
List<String> _vlessCache = [];

Future<void> changeVless() async {
  final values = await bind.mainGetVless();
  // Pre-fill the compiled-in gateway when nothing was configured yet, so
  // enabling VLESS never requires typing the values by hand.
  // Injected at build time via --dart-define=RD_VLESS_SERVER=...,RD_VLESS_UUID=... (see README.md).
  final def = values.isNotEmpty
      ? values
      : const [
          String.fromEnvironment('RD_VLESS_SERVER',
              defaultValue: 'your-domain.example'),
          '443',
          String.fromEnvironment('RD_VLESS_UUID',
              defaultValue: '00000000-0000-0000-0000-000000000000'),
          String.fromEnvironment('RD_VLESS_SERVER',
              defaultValue: 'your-domain.example'),
        ];
  final server = TextEditingController(text: def[0]);
  final port = TextEditingController(text: def.length > 1 ? def[1] : '443');
  final uuid = TextEditingController(text: def.length > 2 ? def[2] : '');
  final sni = TextEditingController(text: def.length > 3 ? def[3] : '');
  var error = '';
  var saving = false;
  gFFI.dialogManager.show((setState, close, context) {
    ...
    submit() async {
      final host = server.text.trim();
      final portValue = int.tryParse(port.text.trim());
      if (host.isNotEmpty &&
          (portValue == null ||
              portValue < 1 ||
              portValue > 65535 ||
              uuid.text.trim().isEmpty ||
              sni.text.trim().isEmpty)) {
        setState(
            () => error = 'Server, port, UUID and TLS domain are required.');
        return;
      }
      ...
      await bind.mainSetVless(
        ...
        port: port.text.trim(),
        uuid: uuid.text.trim(),
        serverName: sni.text.trim(),
      );
      _vlessCache = host.isNotEmpty
          ? [host, port.text.trim(), uuid.text.trim(), sni.text.trim()]
          : [];
      close();
    }

    return CustomAlertDialog(
      title: const Text('VLESS + TCP + TLS'),
      content: ConstrainedBox(
        constraints: const BoxConstraints(minWidth: 460),
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          field('Server (empty to disable VLESS)', server),
          field('Port', port),
          field('UUID', uuid),
          field('TLS domain (SNI)', sni),
          if (error.isNotEmpty)
...
Future<void> changeSsh() async {
  final values = await bind.mainGetSsh();
  // The compiled-in ed25519 identity pre-fills the keys and username.
  final def = values.isNotEmpty
      ? values
      : ['', '22', 'rustdesk', '', ''];
  final server = TextEditingController(text: def[0]);
  final port = TextEditingController(text: def.length > 1 ? def[1] : '22');
  final username =
      TextEditingController(text: def.length > 2 ? def[2] : 'rustdesk');
  final publicKey = TextEditingController(text: def.length > 3 ? def[3] : '');
  final privateKey = TextEditingController(text: def.length > 4 ? def[4] : '');
  var error = '';
  var saving = false;
  gFFI.dialogManager.show((setState, close, context) {
    ...
    submit() async {
      ...
      if (host.isNotEmpty &&
          (portValue == null ||
              ...
              privateKey.text.trim().isEmpty)) {
        setState(() => error =
            'Server, port, username, public key and private key are required.');
        return;
      }
      setState(() => saving = true);
      await bind.mainSetSsh(
    ...
    return CustomAlertDialog(
      title: const Text('SSH + TCP'),
      content: ConstrainedBox(
        constraints: const BoxConstraints(minWidth: 520),
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          field('Server IP (empty to disable SSH)', server),
          field('Port', port),
          field('Username', username),
          field('Public key', publicKey, maxLines: 3),
          field('Private key', privateKey, maxLines: 6),
          if (error.isNotEmpty)
            Text(error, style: const TextStyle(color: Colors.red)),
          if (saving) const LinearProgressIndicator(),
        ]),
