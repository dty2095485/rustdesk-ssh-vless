                      ...
                      _In_ wchar_t *command_line, _In_ int show_command)
{
  ...
  // Connection-manager processes are launched by the RustDesk service and do
  // not inherit environment variables set in the interactive RDP shell. Their
  // small utility UI does not benefit from GPU acceleration, while ANGLE's
  // Direct3D path is known to present a permanently white surface on some RDP
  // virtual adapters. Use software rendering for CM and install windows
  // unconditionally. Keep the normal desktop UI hardware-accelerated outside
  // remote sessions.
  const bool is_cm_process = std::any_of(
      command_line_arguments.begin(), command_line_arguments.end(),
      [](const std::string& argument) {
        return argument.compare(0, 4, "--cm") == 0;
      });
  const bool is_install_process =
      std::find(command_line_arguments.begin(),
                command_line_arguments.end(), "--install") !=
      command_line_arguments.end();
  if (::GetSystemMetrics(SM_REMOTESESSION) != 0 || is_cm_process ||
      is_install_process) {
    ::SetEnvironmentVariableA("ANGLE_DEFAULT_PLATFORM", "swiftshader");
  }
