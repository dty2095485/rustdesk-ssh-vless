...
//
// The timer below drives a bounded recovery pass. Two subtleties, verified
// against the embedder sources (identical in 3.24.5 and 3.44.0):
...
// - Neither the first-frame callback nor GDI pixel sampling reliably proves
//   that a GPU-backed Flutter frame was presented. Recovery therefore never
//   destroys or relaunches a process; it only nudges visible windows for a
//   bounded period.
...
// Stop eventually so a genuinely stuck engine does not keep a timer alive
...
FlutterWindow::MessageHandler(HWND hwnd, UINT const message,
  ...
  switch (message) {
    ...
    case WM_TIMER:
      if (wparam == kForceRedrawTimerId) {
        if (!flutter_controller_) {
          ...
          // Connection-manager and other utility windows are intentionally
          // created hidden. Do not run presentation recovery until they are
          // actually shown; WM_SHOWWINDOW starts a fresh, bounded pass.
          KillTimer(hwnd, kForceRedrawTimerId);
        } else {
          ++force_redraw_tries_;
          if (force_redraw_tries_ <= kForceRedrawCheapTries) {
            flutter_controller_->ForceRedraw();
          } else if (force_redraw_tries_ % 5 == 1) {
            ForceChildRefresh(flutter_controller_->view()->GetNativeWindow());
          }
          if (force_redraw_tries_ >= kForceRedrawMaxTries) {
            KillTimer(hwnd, kForceRedrawTimerId);
          }
        }
        return 0;
    ...
    case WM_SHOWWINDOW:
      ...
      // protection when it becomes visible (see kForceRedrawTimerId).
