...
// Custom build: fall back to the self-hosted server only, never public ones.
// Real values are injected at build time via env vars (see README.md); the
// checked-in defaults are placeholders so a plain `cargo build` still compiles.
pub const RENDEZVOUS_HOST: &str = match option_env!("RD_RENDEZVOUS_SERVER") {
    Some(s) => s,
    None => "your-domain.example",
};
pub const RENDEZVOUS_SERVERS: &[&str] = &[RENDEZVOUS_HOST];
pub const RS_PUB_KEY: &str = match option_env!("RD_RENDEZVOUS_PUBKEY") {
    Some(s) => s,
    None => "",
};
pub const RELAY_KEY: &str = match option_env!("RD_RELAY_KEY") {
    Some(s) => s,
    None => "",
};
pub const VLESS_SERVER: &str = match option_env!("RD_VLESS_SERVER") {
    Some(s) => s,
    None => "your-domain.example",
};
pub const VLESS_UUID: &str = match option_env!("RD_VLESS_UUID") {
    Some(s) => s,
...
pub struct VlessServer {
    ...
    #[serde(default = "default_vless_enabled")]
    pub enabled: bool,
}

fn default_vless_enabled() -> bool {
    true
}

// Built-in SSH identity (ed25519) shared by the client and the hbssh gateway.
// Injected at build time via env vars (see README.md); defaults are empty/placeholder.
pub const SSH_BUILTIN_USERNAME: &str = match option_env!("RD_SSH_USERNAME") {
    Some(s) => s,
    None => "rustdesk",
};
pub const SSH_BUILTIN_PUBLIC_KEY: &str = match option_env!("RD_SSH_PUBLIC_KEY") {
    Some(s) => s,
    None => "",
};
pub const SSH_BUILTIN_PRIVATE_KEY: &str = match option_env!("RD_SSH_PRIVATE_KEY") {
    Some(s) => s,
    None => "",
};
...
// Custom build: fresh installs receive the same self-hosted VLESS and SSH
// endpoints as the reference client.  Device identity and passwords stay
// generated locally; only transport settings are built in.
impl Default for Config2 {
    fn default() -> Self {
        let mut options = HashMap::new();
        options.insert("enable-check-update".to_owned(), "N".to_owned());
        // The UI shows this option in the ID server field.
        options.insert(
            "custom-rendezvous-server".to_owned(),
            RENDEZVOUS_HOST.to_owned(),
        );
        // Direct-connection mode needs the relay server and the server key.
        // No port: 21117 is the official default (check_port appends it);
        // SSH mode wraps the same address through the 22222 tunnel.
        options.insert("relay-server".to_owned(), RENDEZVOUS_HOST.to_owned());
        options.insert("key".to_owned(), RELAY_KEY.to_owned());
        options.insert(keys::OPTION_DISABLE_UDP.to_owned(), "Y".to_owned());
        options.insert("force-always-relay".to_owned(), "Y".to_owned());
        options.insert(keys::OPTION_ALLOW_WEBSOCKET.to_owned(), "N".to_owned());
        Self {
            rendezvous_server: format!("{RENDEZVOUS_HOST}:21116"),
            nat_type: 0,
            serial: 0,
            unlock_pin: "".to_owned(),
            trusted_devices: "".to_owned(),
            socks: None,
            vless: Some(VlessServer {
                server: VLESS_SERVER.to_owned(),
                port: 443,
                uuid: VLESS_UUID.to_owned(),
                server_name: VLESS_SERVER.to_owned(),
                enabled: false,
            }),
            ssh: Some(SshServer {
                server: RENDEZVOUS_HOST.to_owned(),
                port: 2222,
                username: SSH_BUILTIN_USERNAME.to_owned(),
                public_key: SSH_BUILTIN_PUBLIC_KEY.to_owned(),
                private_key: SSH_BUILTIN_PRIVATE_KEY.to_owned(),
                enabled: true,
            }),
            options,
        }
...
impl Config {
    ...
    pub fn set_vless(vless: Option<VlessServer>) {
        // SSH and VLESS are mutually exclusive; enabling one disables the other.
        if vless.as_ref().map(|v| v.enabled).unwrap_or(false) {
            if let Some(mut ssh) = Self::get_ssh_config() {
                if ssh.enabled {
                    ssh.enabled = false;
                    Self::set_ssh(Some(ssh));
                }
            }
        }
        let mut config = CONFIG2.write().unwrap();
        let was_enabled = config.vless.as_ref().map(|v| v.enabled).unwrap_or(false);
        let enabled = vless.as_ref().map(|v| v.enabled).unwrap_or(false);
        if config.vless != vless {
            config.vless = vless;
            config.store();
        }
        drop(config);
        if was_enabled != enabled {
            Self::set_vless_mode(enabled);
        }
    }

    pub fn get_vless_config() -> Option<VlessServer> {
        CONFIG2.read().unwrap().vless.clone()
    }

    pub fn get_vless() -> Option<VlessServer> {
        Self::get_vless_config()
            .filter(|v| v.enabled && !v.server.is_empty() && v.port > 0 && !v.uuid.is_empty())
    }

    pub fn set_ssh(ssh: Option<SshServer>) {
        // SSH and VLESS are mutually exclusive; enabling one disables the other.
        if ssh.as_ref().map(|s| s.enabled).unwrap_or(false) {
            if let Some(mut vless) = Self::get_vless_config() {
                if vless.enabled {
                    vless.enabled = false;
                    Self::set_vless(Some(vless));
                }
            }
        }
        let mut config = CONFIG2.write().unwrap();
        let was_enabled = config.ssh.as_ref().map(|s| s.enabled).unwrap_or(false);
        let enabled = ssh.as_ref().map(|s| s.enabled).unwrap_or(false);
        if config.ssh != ssh {
            config.ssh = ssh;
            config.store();
        }
        drop(config);
        if was_enabled != enabled {
            Self::set_ssh_mode(enabled);
        }
    }

    pub fn get_ssh_config() -> Option<SshServer> {
        CONFIG2.read().unwrap().ssh.clone().or_else(|| {
            Some(SshServer {
                server: "".to_owned(),
                port: 22,
                username: SSH_BUILTIN_USERNAME.to_owned(),
    ...
    pub fn get_ssh() -> Option<SshServer> {
        Self::get_ssh_config().filter(|s| {
            s.enabled
                && !s.server.is_empty()
                && s.port > 0
                && !s.username.is_empty()
    ...
    fn set_ssh_mode(enabled: bool) {
        const SAVED_DISABLE_UDP: &str = "ssh-saved-disable-udp";
        const SAVED_FORCE_RELAY: &str = "ssh-saved-force-always-relay";
        const SAVED_WEBSOCKET: &str = "ssh-saved-allow-websocket";

        if enabled {
            Self::set_option(
                SAVED_DISABLE_UDP.to_owned(),
                Self::get_option(keys::OPTION_DISABLE_UDP),
            );
            Self::set_option(
                SAVED_FORCE_RELAY.to_owned(),
                Self::get_option("force-always-relay"),
            );
            Self::set_option(
                SAVED_WEBSOCKET.to_owned(),
                Self::get_option(keys::OPTION_ALLOW_WEBSOCKET),
            );
            Self::set_option(keys::OPTION_DISABLE_UDP.to_owned(), "Y".to_owned());
            Self::set_option("force-always-relay".to_owned(), "Y".to_owned());
            Self::set_option(keys::OPTION_ALLOW_WEBSOCKET.to_owned(), "N".to_owned());
        } else {
            let disable_udp = Self::get_option(SAVED_DISABLE_UDP);
            let force_relay = Self::get_option(SAVED_FORCE_RELAY);
            let websocket = Self::get_option(SAVED_WEBSOCKET);
            Self::set_option(keys::OPTION_DISABLE_UDP.to_owned(), disable_udp);
            Self::set_option("force-always-relay".to_owned(), force_relay);
            Self::set_option(keys::OPTION_ALLOW_WEBSOCKET.to_owned(), websocket);
            Self::set_option(SAVED_DISABLE_UDP.to_owned(), "".to_owned());
            Self::set_option(SAVED_FORCE_RELAY.to_owned(), "".to_owned());
            Self::set_option(SAVED_WEBSOCKET.to_owned(), "".to_owned());
        }
    }

    fn set_vless_mode(enabled: bool) {
        const SAVED_DISABLE_UDP: &str = "vless-saved-disable-udp";
        const SAVED_FORCE_RELAY: &str = "vless-saved-force-always-relay";
        const SAVED_WEBSOCKET: &str = "vless-saved-allow-websocket";
    ...
    pub fn get_network_type() -> NetworkType {
        if Self::get_vless().is_some() {
            return NetworkType::Vless;
        }
        if Self::get_ssh().is_some() {
            return NetworkType::Ssh;
        }
        if OVERWRITE_SETTINGS
            .read()
            .unwrap()
