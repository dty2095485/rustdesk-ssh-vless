...
impl FramedSocket {
    ...
    // Pin the egress route: on multi-homed hosts (dual WAN/VPN) an
    // unconnected UDP socket can send via a path whose replies never come
    // back, which makes registration fail while connected UDP works.
    #[inline]
    pub async fn connect(&mut self, addr: SocketAddr) -> ResultType<()> {
        match self {
            Self::Direct(f) => {
                f.get_mut().connect(addr).await?;
            }
            Self::ProxySocks(_) => {}
        };
        Ok(())
