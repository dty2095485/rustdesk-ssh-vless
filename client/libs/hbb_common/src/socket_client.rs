...
>(
    ...
    if let Some(conf) = Config::get_vless() {
        if local.is_some() {
            log::debug!("VLESS transport does not preserve the requested local address");
        }
        return Ok(Stream::Tcp(
            vless::connect(&target.to_string(), &conf, ms_timeout).await?,
        ));
    }

    if let Some(conf) = Config::get_ssh() {
        if local.is_some() {
            log::debug!("SSH transport does not preserve the requested local address");
        }
        return Ok(Stream::Tcp(
            ssh::connect(&target.to_string(), &conf, ms_timeout).await?,
        ));
    }
...
pub async fn new_udp_for(
    ...
    // Registration UDP is always direct; VLESS only wraps the TCP relay,
    // so resolve the target in every mode. Only the SOCKS proxy keeps the
    // hostname (the proxy resolves it).
    let (ipv4, target) = if Config::get_socks().is_some() {
        (true, target.into_target_addr()?)
    } else {
        let addr = test_target(target).await?;
        (addr.is_ipv4(), addr.into_target_addr()?)
    };
    let mut socket = new_udp(Config::get_any_listen_addr(ipv4), ms_timeout).await?;
    // Connect so the egress route is pinned (see FramedSocket::connect);
    // unconnected sends can leave via a WAN whose replies never arrive.
    if let TargetAddr::Ip(addr) = &target {
        socket.connect(*addr).await?;
    }
    Ok((socket, target.to_owned()))
}
...
pub async fn rebind_udp_for(
    ...
    // VLESS registration UDP is direct too; only SOCKS skips the rebind.
    if Config::get_socks().is_some() {
        return Ok(None);
    }
    let addr = test_target(target).await?;
    let v4 = addr.is_ipv4();
    let mut socket = FramedSocket::new(Config::get_any_listen_addr(v4)).await?;
    socket.connect(addr).await?;
    Ok(Some((socket, addr.into_target_addr()?.to_owned())))
}
