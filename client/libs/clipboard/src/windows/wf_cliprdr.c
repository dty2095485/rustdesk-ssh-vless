...
/*
 * Explorer often asks for a remote clipboard file in small ranges.  The
 * CLIPRDR request/response protocol is synchronous, so use a bounded cache
 * to amortize one network round trip across several sequential Read calls.
 */
/* Keep the compatibility path deliberately small: it shares the interactive
 * desktop connection. Large clipboard files use the optional high-speed path
 * when both endpoints support it; otherwise this bounded fallback remains
 * responsive. */
#define CLIPRDR_STREAM_PREFETCH_BYTES (256U * 1024U)
#define CLIPRDR_FAST_STREAM_PREFETCH_BYTES (2U * 1024U * 1024U)
...
struct wf_clipboard
{
	...
	HANDLE req_fevent;
	BOOL req_f_received;
	SRWLOCK req_f_state_lock;
	BOOL req_f_inflight;
	BOOL req_f_inflight_fast;
	ULONG req_f_generation_seq;
	ULONG req_f_generation_expected;
	ULONG req_f_generation_completed;
	ULONG req_f_generation_failed;
	...
	LONG fast_conn_id;                  // connection allowed to use the larger prefetch window
...
static UINT cliprdr_send_request_filecontents(wfClipboard *clipboard, UINT32 connID, UINT32 streamId,
											  ...
											  DWORD positionlow, ULONG request, BYTE **response_data,
											  ULONG *response_size);
...
static void CliprdrStream_ClearCache(CliprdrStream *instance)
{
	if (!instance)
		return;

	free(instance->m_cache);
	instance->m_cache = NULL;
	instance->m_cache_len = 0;
	instance->m_cache_pos = 0;
	instance->m_cache_offset = 0;
}
													...
													ULONG *pcbRead)
{
	UINT ret;
	ULONG requested;
	ULONG prefetch_bytes;
	ULONG available;
	ULONG to_copy;
	ULONGLONG remaining;
	BYTE *response_data = NULL;
	ULONG response_size = 0;
	CliprdrStream *instance = (CliprdrStream *)This;
	...
	*pcbRead = 0;
	...
	if (instance->m_cache &&
		instance->m_cache_offset + instance->m_cache_pos != instance->m_lOffset.QuadPart)
	{
		CliprdrStream_ClearCache(instance);
	}

	if (!instance->m_cache || instance->m_cache_pos >= instance->m_cache_len)
	{
		CliprdrStream_ClearCache(instance);
		remaining = instance->m_lSize.QuadPart - instance->m_lOffset.QuadPart;
		prefetch_bytes =
			((UINT32)InterlockedCompareExchange(&clipboard->fast_conn_id, 0, 0) ==
			 instance->m_connID)
				? CLIPRDR_FAST_STREAM_PREFETCH_BYTES
				: CLIPRDR_STREAM_PREFETCH_BYTES;
		requested = cb > prefetch_bytes ? cb : prefetch_bytes;
		if (remaining < requested)
			requested = (ULONG)remaining;
		...
		ret = cliprdr_send_request_filecontents(clipboard, instance->m_connID, instance->m_streamId,
																...
																instance->m_lOffset.LowPart, requested,
		...
		if (response_size > requested)
		{
			free(response_data);
			return STG_E_READFAULT;
		}

		instance->m_cache = response_data;
		instance->m_cache_len = response_size;
		instance->m_cache_pos = 0;
		instance->m_cache_offset = instance->m_lOffset.QuadPart;
		response_data = NULL;
		if (!instance->m_cache || instance->m_cache_len == 0)
		{
			CliprdrStream_ClearCache(instance);
			return S_FALSE;
		}
	...
	available = instance->m_cache_len - instance->m_cache_pos;
	to_copy = cb < available ? cb : available;
	CopyMemory(pv, instance->m_cache + instance->m_cache_pos, to_copy);
	instance->m_cache_pos += to_copy;
	*pcbRead = to_copy;
	// Check overflow, can not be a real case
	if ((instance->m_lOffset.QuadPart + to_copy) < instance->m_lOffset.QuadPart) {
	...
	instance->m_lOffset.QuadPart += to_copy;
	if (instance->m_cache_pos == instance->m_cache_len)
		CliprdrStream_ClearCache(instance);
	...
	if (instance)
	{
		...
		if (iStream->lpVtbl)
		{
			...
			if (((instance->m_Dsc.dwFlags & FD_FILESIZE) == 0) && !isDir)
			{
				...
				{
					LONGLONG sz = 0;
					CopyMemory(&sz, response_data, sizeof(sz));
					if (sz < 0)
						success = FALSE;
					else
...
static UINT cliprdr_send_request_filecontents(wfClipboard *clipboard, UINT32 connID, UINT32 streamId, ULONG index,
									   ...
									   ULONG nreq, BYTE **response_data, ULONG *response_size)
{
	UINT rc;
	DWORD mutex_result;
	ULONG generation;
	UINT32 request_stream_id;
	...
	if (!clipboard || !clipboard->context || !clipboard->context->ClientFileContentsRequest ||
		!response_data || !response_size || !clipboard->req_fmutex)
		return ERROR_INTERNAL_ERROR;

	*response_data = NULL;
	*response_size = 0;
	mutex_result = WaitForSingleObject(clipboard->req_fmutex, INFINITE);
	if (mutex_result != WAIT_OBJECT_0 && mutex_result != WAIT_ABANDONED)
		return ERROR_INTERNAL_ERROR;

	/* A wire stream id is unique per request, rather than merely per IStream.
	 * A response delayed beyond cancellation therefore cannot satisfy the next
	 * request made by the same Explorer stream. */
	(void)streamId;
	request_stream_id = (UINT32)InterlockedIncrement(&clipboard->req_f_stream_id_seq);
	if (request_stream_id == 0)
		request_stream_id = (UINT32)InterlockedIncrement(&clipboard->req_f_stream_id_seq);

	AcquireSRWLockExclusive(&clipboard->req_f_state_lock);
	rc = try_reset_event(clipboard->req_fevent);
	if (rc != ERROR_SUCCESS)
	{
		ReleaseSRWLockExclusive(&clipboard->req_f_state_lock);
		ReleaseMutex(clipboard->req_fmutex);
		return rc;
	}
	...
	generation = ++clipboard->req_f_generation_seq;
	if (generation == 0)
		generation = ++clipboard->req_f_generation_seq;
	clipboard->req_f_generation_expected = generation;
	clipboard->req_f_generation_completed = 0;
	clipboard->req_f_generation_failed = 0;
	clipboard->req_f_conn_id_expected = connID;
	clipboard->req_f_stream_id_expected = request_stream_id;
	clipboard->req_fsize_expected = nreq;
	clipboard->req_f_inflight = TRUE;
	clipboard->req_f_inflight_fast =
		((UINT32)InterlockedCompareExchange(&clipboard->fast_conn_id, 0, 0) == connID);
	ReleaseSRWLockExclusive(&clipboard->req_f_state_lock);

	fileContentsRequest.connID = connID;
	fileContentsRequest.streamId = request_stream_id;
	...
	{
		AcquireSRWLockExclusive(&clipboard->req_f_state_lock);
		if (clipboard->req_f_generation_expected == generation)
		{
			clipboard->req_f_inflight = FALSE;
			clipboard->req_f_generation_failed = generation;
		}
		ReleaseSRWLockExclusive(&clipboard->req_f_state_lock);
		ReleaseMutex(clipboard->req_fmutex);
		return rc;
	...
	if (clipboard->req_f_generation_expected != generation ||
		clipboard->req_f_generation_failed == generation ||
		clipboard->req_f_generation_completed != generation)
	{
		rc = ERROR_INTERNAL_ERROR;
	}
	else if (rc == CHANNEL_RC_OK)
	{
		*response_data = (BYTE *)clipboard->req_fdata;
		*response_size = clipboard->req_fsize;
		clipboard->req_fdata = NULL;
		clipboard->req_fsize = 0;
	}
	if (clipboard->req_f_generation_expected == generation)
										 ...
										 const CLIPRDR_FILE_CONTENTS_RESPONSE *fileContentsResponse)
{
	...
	{
		/* A cancelled or previous-generation response must not wake the next request. */
		ReleaseSRWLockExclusive(&clipboard->req_f_state_lock);
		return CHANNEL_RC_OK;
	}
	generation = clipboard->req_f_generation_expected;
	free(clipboard->req_fdata);
	clipboard->req_fsize = 0;
	clipboard->req_fdata = NULL;
...
BOOL wf_cliprdr_init(wfClipboard *clipboard, CliprdrClientContext *cliprdr)
{
	...
	InitializeSRWLock(&clipboard->req_f_state_lock);
	clipboard->req_f_inflight = FALSE;
	clipboard->req_f_inflight_fast = FALSE;
	clipboard->req_f_generation_seq = 0;
	clipboard->req_f_generation_expected = 0;
	clipboard->req_f_generation_completed = 0;
	clipboard->req_f_generation_failed = 0;
	clipboard->req_f_stream_id_seq = 0;
	...
	if (!(clipboard->req_fmutex = CreateMutex(NULL, FALSE, NULL)))
		goto error;
...
BOOL wf_cliprdr_uninit(wfClipboard *clipboard, CliprdrClientContext *cliprdr)
{
	...
	if (clipboard->req_fmutex)
	{
		CloseHandle(clipboard->req_fmutex);
		clipboard->req_fmutex = NULL;
	}
...
BOOL set_fast_cliprdr(CliprdrClientContext *context, UINT32 connID, BOOL enabled)
{
	wfClipboard *clipboard = NULL;
	LONG previous_fast_conn_id;
	...
	if (enabled)
	{
		InterlockedExchange(&clipboard->fast_conn_id, (LONG)connID);
	}
	else
	{
		/* Do not let a stale connection disable a newer fast channel. Only the
		 * connection that wins this comparison is allowed to fail its own
		 * outstanding request. */
		AcquireSRWLockExclusive(&clipboard->req_f_state_lock);
		previous_fast_conn_id =
			InterlockedCompareExchange(&clipboard->fast_conn_id, 0, (LONG)connID);
		if ((UINT32)previous_fast_conn_id == connID)
		{
			if (clipboard->req_f_inflight && clipboard->req_f_inflight_fast &&
				clipboard->req_f_conn_id_expected == connID)
			{
				/* The fast transport has already owned this request. Wake Explorer
				 * with an explicit failure instead of retrying it on the interactive
				 * channel or leaving it blocked until the response timeout. */
				free(clipboard->req_fdata);
				clipboard->req_fdata = NULL;
				clipboard->req_fsize = 0;
				clipboard->req_f_generation_completed = 0;
				clipboard->req_f_generation_failed =
					clipboard->req_f_generation_expected;
				clipboard->req_f_inflight = FALSE;
				if (!SetEvent(clipboard->req_fevent))
				{
					DEBUG_CLIPRDR("set_fast_cliprdr(), SetEvent failed with 0x%x",
								 GetLastError());
					clipboard->req_f_received = TRUE;
				}
			}
