...
pub trait CliprdrServiceContext: Send + Sync {
    ...
    /// Mark a connection as the optional high-throughput clipboard channel.
    /// Platforms without a separate fast channel keep the compatibility behavior.
    fn set_fast_channel(&mut self, _conn_id: i32, _enabled: bool) -> Result<(), CliprdrError> {
        Ok(())
    }
