...
impl CliprdrServiceContext for CliprdrClientContext {
    ...
    fn set_fast_channel(&mut self, conn_id: i32, enabled: bool) -> Result<(), CliprdrError> {
        set_fast_cliprdr_channel(self, conn_id, enabled)
    }
...
pub fn set_fast_cliprdr_channel(
    context: &mut CliprdrClientContext,
    conn_id: i32,
    enabled: bool,
) -> Result<(), CliprdrError> {
    if conn_id <= 0 {
        return Err(CliprdrError::InvalidRequest {
            description: format!("invalid fast clipboard connection id: {conn_id}"),
        });
    }
