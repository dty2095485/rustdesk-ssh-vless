                    ...
                    match res {
                        ...
                        Ok(Some(data)) => {
                            match data {
                                ...
                                #[cfg(target_os = "windows")]
                                Data::ClipboardFastChannel(enabled) => {
                                    let conn_id = self.conn_id;
                                    if let Err(err) = ContextSend::proc(|context| {
                                        context
                                            .set_fast_channel(conn_id, enabled)
                                            .map_err(|e| e.into())
                                    }) {
                                        log::error!(
                                            "failed to set fast clipboard channel for connection {}: {}",
                                            conn_id,
                                            err
                                        );
                                        if enabled {
                                            // Tell the service to remove the fast child mapping.
                                            // Otherwise it would keep routing file-content frames
                                            // into a channel this clipboard context never enabled.
                                            self.tx
                                                .send(Data::ClipboardFastChannel(false))
                                                .ok();
                                        }
                                    }
                                }
