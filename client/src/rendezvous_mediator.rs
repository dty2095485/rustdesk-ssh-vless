...
impl RendezvousMediator {
    ...
    pub async fn start_tcp(server: ServerPtr, host: String) -> ResultType<()> {
        ...
        // Only run the key exchange when the server actually has a key; the
        // fallback public key would otherwise stall the handshake on keyless
        // servers. Under VLESS or SSH the tunnel already encrypts, and the
        // server does not run the KeyExchange handshake, so skip it there too.
        let key = Config::get_option("key");
        if !key.is_empty() && Config::get_vless().is_none() && Config::get_ssh().is_none() {
        ...
        loop {
            ...
            select! {
                ...
                _ = timer.tick() => {
                    ...
                    // register_peer sends RegisterPk while unconfirmed and
                    // refreshes the registration (RegisterPeer) afterwards,
                    // mirroring the UDP path so the server keeps us online.
    ...
    pub async fn start(server: ServerPtr, host: String) -> ResultType<()> {
        ...
        // Plan A: VLESS only wraps the relay/punch transport; registration
        // stays on the official UDP path so the official server works as-is.
