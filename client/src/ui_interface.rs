...
pub fn use_texture_render() -> bool {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // Flutter's external texture path depends on ANGLE/Direct3D.  When the
        // runner selects SwiftShader (for example to avoid a white Flutter UI
        // on an RDP virtual display), ordinary widgets render correctly but
        // the external video texture can stay permanently black.  Keep the
        // SwiftShader compatibility for the window itself and send decoded
        // frames through RustDesk's pixel-buffer renderer instead.
        if std::env::var("ANGLE_DEFAULT_PLATFORM")
            .map(|value| value.eq_ignore_ascii_case("swiftshader"))
            .unwrap_or(false)
        {
            return false;
        }
...
#[inline]
pub fn get_vless() -> Vec<String> {
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    let v = ipc::get_vless();
    #[cfg(any(target_os = "android", target_os = "ios"))]
    let v = Config::get_vless_config();
    match v {
        None => Vec::new(),
        Some(v) => vec![
            v.server,
...
#[inline]
pub fn set_vless(server: String, port: String, uuid: String, server_name: String) {
    let vless = if server.is_empty() {
        Config::get_vless_config().map(|mut v| {
            v.enabled = false;
            v
        })
    } else {
        Some(config::VlessServer {
            server,
    ...
    let Some(vless) = vless else {
        return;
    };
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    ipc::set_vless(vless).ok();
    #[cfg(any(target_os = "android", target_os = "ios"))]
    {
        let _nat = crate::CheckTestNatType::new();
        Config::set_vless(Some(vless));
        crate::RendezvousMediator::restart();
    }
}

#[inline]
pub fn get_ssh() -> Vec<String> {
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    let s = ipc::get_ssh();
    #[cfg(any(target_os = "android", target_os = "ios"))]
    let s = Config::get_ssh_config();
    match s {
        None => Vec::new(),
        Some(s) => vec![
            s.server,
...
pub fn set_ssh(
    ...
    let ssh = if server.is_empty() {
        Config::get_ssh_config().map(|mut s| {
            s.enabled = false;
            s
        })
    } else {
        Some(config::SshServer {
            server,
            port: port.parse().unwrap_or_default(),
            username,
            public_key,
...
pub fn is_installed_lower_version() -> bool {
    ...
    #[cfg(windows)]
    {
        // Custom build: never prompt "installed version is lower".
        let _ = crate::BUILD_DATE;
        return false;
    }
}
