...
impl Connection {
        ...
        loop {
            tokio::select! {
                    ...
                    match data {
                        ...
                        ipc::Data::ClipboardFile(clip) => {
                            ...
                            match clip {
                                ...
                                clip if Self::is_clipboard_fast_payload(&clip) => {
                                    let routed = match raii::AuthedConnID::clipboard_fast_sender(
                                        &conn.session_key(),
                                    ) {
                                        Some(sender) => sender
                                            .send(ipc::Data::FastClipboard(clip))
                                            .map_err(|err| err.0),
                                        None => Err(ipc::Data::FastClipboard(clip)),
                                    };
                                    if let Err(ipc::Data::FastClipboard(clip)) = routed {
                                        if let Err(err) =
                                            conn.stream.send(&clip_2_msg(clip)).await
                                        {
                        ...
                        #[cfg(target_os = "windows")]
                        ipc::Data::ClipboardFastChannel(false) => {
                            if conn.is_remote() {
                                if let Some(sender) = raii::AuthedConnID::clipboard_fast_sender(
                                    &conn.session_key(),
                                ) {
                                    sender.send(ipc::Data::ClipboardFastChannel(false)).ok();
                                }
                            }
                        }
                ...
                Some(data) = rx_from_authed.recv() => {
                    match data {
                        #[cfg(target_os = "windows")]
                        ipc::Data::FastClipboard(clip) => {
                            if conn.clipboard_fast {
                                if let Err(err) = conn.stream.send(&clip_2_msg(clip)).await {
                                    conn.on_close(&err.to_string(), false).await;
                                    break;
                                }
                            } else if conn.is_remote() {
                                conn.send_to_cm(ipc::Data::ClipboardFile(clip));
                            }
                        }
                        #[cfg(target_os = "windows")]
                        ipc::Data::ClipboardFastChannel(enabled) => {
                            if conn.is_remote() {
                                conn.send_to_cm(ipc::Data::ClipboardFastChannel(enabled));
                            } else if conn.clipboard_fast && !enabled {
                                conn.on_close("clipboard fast channel unavailable", false)
                                    .await;
                                break;
                            }
                        }
    ...
    async fn post_audit_async(url: String, v: Value) -> ResultType<String> {
        ...
        loop {
            ...
            if !retryable || attempt >= ATTEMPTS || elapsed >= RETRY_DEADLINE {
                // A basic server has no api endpoint, so the audit post can
                // never succeed there; failing it must not kill the session.
                log::warn!(
    ...
    async fn send_logon_response_and_keep_alive(&mut self) -> bool {
        ...
        if self.clipboard_fast {
            let session_key = self.session_key();
            if !raii::AuthedConnID::has_remote_session(&session_key)
                || raii::AuthedConnID::has_clipboard_fast_session(&session_key)
            {
        ...
        #[cfg(target_os = "windows")]
        {
            platform_additions.insert("clipboard_fast_channel".into(), json!(true));
    ...
    #[cfg(target_os = "windows")]
    fn is_clipboard_fast_payload(clip: &clipboard::ClipboardFile) -> bool {
        matches!(
            clip,
    ...
    #[cfg(target_os = "windows")]
    fn write_permanent_password_diagnostic(&self, outcome: &str) {
        // Keep enough information to distinguish a changing host salt from a
        // stale saved client credential. Never write a password, challenge,
        // or password-derived credential to disk.
        ...
        let (storage, storage_salt) = Config::get_local_permanent_password_storage_and_salt();
        let diagnostic_dir = std::path::Path::new(r"C:\ProgramData\RustDesk");
        if std::fs::create_dir_all(diagnostic_dir).is_err() {
            return;
        }
        if let Ok(mut file) = OpenOptions::new()
            .create(true)
            .append(true)
            .open(diagnostic_dir.join("permanent-password-auth.log"))
        {
            let _ = writeln!(
                file,
                "connection_id={} outcome={} handshake_salt={} storage_salt={} salts_match={} storage_usable={}",
                self.inner.id(),
                outcome,
                fingerprint(&self.hash.salt),
                fingerprint(&storage_salt),
                self.hash.salt == storage_salt,
                local_permanent_password_storage_is_usable_for_auth(&storage, &storage_salt),
            );
        }
    }
    ...
    fn validate_password(&mut self, allow_permanent_password: bool) -> bool {
        ...
        if password::permanent_enabled() || allow_permanent_password {
            ...
            if !local_storage.is_empty() {
                ...
                {
                    #[cfg(target_os = "windows")]
                    self.write_permanent_password_diagnostic("accepted");
                    // The temporary-password branch creates a session liveness
                    // record before starting the connection manager. Keep the
                    // same non-secret record for permanent-password sessions:
                    // it supplies the shared receive timestamp used when the
                    // Windows connection manager is recreated, without making
                    // a later login eligible for passwordless re-auth.
                    raii::AuthedConnID::update_or_insert_session(
                        self.session_key(),
                        None,
                        Some(false),
        ...
        #[cfg(target_os = "windows")]
        if !self.lr.password.is_empty() && password::permanent_enabled() {
            self.write_permanent_password_diagnostic("rejected");
        }
        false
    }
        ...
        if let Some(message::Union::LoginRequest(lr)) = msg.union {
            ...
            match lr.union {
                Some(login_request::Union::FileTransfer(ft)) => {
                    ...
                    self.clipboard_fast = ft.clipboard_fast && cfg!(target_os = "windows");
                ...
                Some(message::Union::Cliprdr(clip)) => {
                    ...
                    if self.clipboard_fast {
                        if let Some(clip) = msg_2_clip(clip) {
                            let Some(sender) =
                                raii::AuthedConnID::remote_sender(&self.session_key())
                            else {
                                return false;
                            };
                            if sender.send(ipc::Data::FastClipboard(clip)).is_err() {
                                return false;
                            }
                        }
                        return true;
    ...
    async fn on_close(&mut self, reason: &str, lock: bool) {
        ...
        #[cfg(target_os = "windows")]
        {
            // The service runs under a different Windows account, so its normal
            // log is not visible from the desktop session. Keep a minimal,
            // non-sensitive closure trace in ProgramData while diagnosing
            // transport and session failures.
            let diagnostic_dir = std::path::Path::new(r"C:\ProgramData\RustDesk");
            if std::fs::create_dir_all(diagnostic_dir).is_ok() {
                if let Ok(mut file) = OpenOptions::new()
                    .create(true)
                    .append(true)
                    .open(diagnostic_dir.join("connection-close.log"))
                {
                    let _ = writeln!(file, "connection_id={} reason={}", self.inner.id(), reason);
                }
            }
        }
    ...
    fn authorized_scope_violation(&self, msg: &Message) -> Option<&'static str> {
        ...
        Self::authorized_message_scope_violation(conn_type, msg, self.clipboard_fast)
    }
    ...
    fn authorized_message_scope_violation(
        ...
        let noop_compat = match conn_type {
            AuthConnType::FileTransfer if clipboard_fast => false,
        ...
        let allowed = match conn_type {
            ...
            AuthConnType::FileTransfer if clipboard_fast => {
                Self::is_clipboard_fast_scoped_message(msg)
            }
    ...
    fn is_clipboard_fast_scoped_message(msg: &Message) -> bool {
        let Some(message::Union::Cliprdr(clip)) = msg.union.as_ref() else {
            return false;
        };
        matches!(
            clip.union.as_ref(),
            Some(cliprdr::Union::FileContentsRequest(_) | cliprdr::Union::FileContentsResponse(_))
        )
    }
...
async fn start_ipc(
    ...
    // A CM pipe can be reset transiently while Windows recreates the desktop
    // connection-manager process. Keep the initial login message so a new CM
    // pipe can resume the same incoming session instead of dropping it.
    let mut last_login: Option<ipc::Data> = None;
    loop {
        tokio::select! {
            res = stream.next() => {
                match res {
                    Err(err) => {
                        #[cfg(target_os = "windows")]
                        if err.to_string().contains("reset by the peer")
                            || err.to_string().contains("reset by peer")
                        {
                            let mut recovered = false;
                            // First give an already-running CM a short chance to
                            // reopen its listener. If its pipe was torn down with
                            // the desktop process, launch one replacement and
                            // wait through the same readiness window used during
                            // the initial CM bootstrap.
                            for attempt in 0..20 {
                                if attempt == 3 {
                                    match crate::platform::run_as_user(vec!["--cm"]) {
                                        Ok(Some(task)) => {
                                            super::CHILD_PROCESS.lock().unwrap().push(task);
                                            log::info!("Restarted connection manager after IPC reset");
                                        }
                                        Ok(None) => {
                                            log::info!("Requested connection-manager restart after IPC reset");
                                        }
                                        Err(restart_err) => {
                                            log::warn!(
                                                "Failed to restart connection manager after IPC reset: {}",
                                                restart_err
                                            );
                                        }
                                    }
                                ...
                                sleep(0.5).await;
                                if let Ok(mut replacement) = crate::ipc::connect(1_000, "_cm").await {
                                    if let Some(login) = last_login.as_ref() {
                                        if replacement.send(login).await.is_err() {
                                            continue;
                                        }
                                    }
                                    stream = replacement;
                                    recovered = true;
                                    log::info!("Recovered reset connection-manager IPC pipe");
                                    break;
                                }
                            }
                            if recovered {
            ...
            res = rx_to_cm.recv() => {
                match res {
                    Some(data) => {
                        if matches!(data, Data::Login { .. }) {
                            last_login = Some(data.clone());
                        }
...
mod raii {
    ...
    impl AuthedConnID {
        pub fn has_remote_session(key: &SessionKey) -> bool {
            AUTHED_CONNS
                .lock()
                .unwrap()
                .iter()
                .any(|conn| conn.conn_type == AuthConnType::Remote && conn.session_key == *key)
        }

        pub fn has_clipboard_fast_session(key: &SessionKey) -> bool {
            AUTHED_CONNS
                .lock()
                .unwrap()
                .iter()
                .any(|conn| conn.clipboard_fast && conn.session_key == *key)
        }

        pub fn remote_sender(key: &SessionKey) -> Option<mpsc::UnboundedSender<Data>> {
            AUTHED_CONNS
                .lock()
                .unwrap()
                .iter()
                .rev()
                .find(|conn| conn.conn_type == AuthConnType::Remote && conn.session_key == *key)
                .map(|conn| conn.sender.clone())
        }

        #[cfg(target_os = "windows")]
        pub fn clipboard_fast_sender(key: &SessionKey) -> Option<mpsc::UnboundedSender<Data>> {
            AUTHED_CONNS
                .lock()
                .unwrap()
                .iter()
                .rev()
                .find(|conn| conn.clipboard_fast && conn.session_key == *key)
                .map(|conn| conn.sender.clone())
        }
        ...
        pub fn new(
            ...
            let clipboard_fast = matches!(
                lr.union.as_ref(),
                Some(login_request::Union::FileTransfer(ft)) if ft.clipboard_fast
            ) && cfg!(target_os = "windows");
            let fast_session_key = clipboard_fast.then(|| session_key.clone());
            ...
            #[cfg(target_os = "windows")]
            if let Some(session_key) = fast_session_key {
                if let Some(sender) = Self::remote_sender(&session_key) {
                    let _ = sender.send(ipc::Data::ClipboardFastChannel(true));
                }
            }
            #[cfg(not(target_os = "windows"))]
            let _ = fast_session_key;
    ...
    impl Drop for AuthedConnID {
        fn drop(&mut self) {
            ...
            let clipboard_fast_session = {
                let mut conns = AUTHED_CONNS.lock().unwrap();
                let session = conns
                    .iter()
                    .find(|conn| conn.conn_id == self.0 && conn.clipboard_fast)
                    .map(|conn| conn.session_key.clone());
                conns.retain(|conn| conn.conn_id != self.0);
                session
            };
            #[cfg(target_os = "windows")]
            if let Some(session_key) = clipboard_fast_session {
                if let Some(sender) = Self::remote_sender(&session_key) {
                    let _ = sender.send(ipc::Data::ClipboardFastChannel(false));
                }
            }
            #[cfg(not(target_os = "windows"))]
            let _ = clipboard_fast_session;
            let remote_count = AUTHED_CONNS
                .lock()
                .unwrap()
                .iter()
...
mod test {
    ...
    fn assert_scopes(
        ...
        for (msg, expected) in cases {
            assert_eq!(
                Connection::authorized_message_scope_violation(conn_type, &msg, false),
                expected
            );
        }
    }
    ...
    #[test]
    fn clipboard_fast_scope_allows_only_file_content_messages() {
        let request = msg(|msg| {
            let mut clip = Cliprdr::new();
            clip.set_file_contents_request(CliprdrFileContentsRequest::new());
            msg.set_cliprdr(clip);
        });
        let response = msg(|msg| {
            let mut clip = Cliprdr::new();
            clip.set_file_contents_response(CliprdrFileContentsResponse::new());
            msg.set_cliprdr(clip);
        });
        let format_list = msg(|msg| {
            let mut clip = Cliprdr::new();
            clip.set_format_list(CliprdrServerFormatList::new());
            msg.set_cliprdr(clip);
        });

        assert_eq!(
            Connection::authorized_message_scope_violation(
                AuthConnType::FileTransfer,
                &request,
                true,
            ),
        ...
        assert_eq!(
            Connection::authorized_message_scope_violation(
                AuthConnType::FileTransfer,
                &msg(|msg| msg.set_file_action(FileAction::new())),
                true,
            ),
            Some("file_action")
        );
    ...
    fn session_scope_limited_render_noop_options_reject_mixed_fields() {
        for conn_type in [
            ...
            assert_eq!(
                Connection::authorized_message_scope_violation(conn_type, &mixed_option, false),
                Some("misc.option")
            );
        }
    }
