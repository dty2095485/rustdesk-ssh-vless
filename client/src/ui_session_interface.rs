...
pub struct Session<T: InvokeUiSession> {
    ...
    /// This is a hidden FILE_TRANSFER session dedicated to Windows clipboard
    /// file-content requests and responses.
    #[cfg(target_os = "windows")]
    pub clipboard_fast: bool,
    /// Clipboard connection id owned by the visible/default parent session.
    #[cfg(target_os = "windows")]
    pub clipboard_parent_conn_id: i32,
    /// Sender installed by the hidden child io loop and shared with its parent.
    #[cfg(target_os = "windows")]
    pub clipboard_fast_sender: Arc<RwLock<Option<mpsc::UnboundedSender<Data>>>>,
    /// Becomes true only after the hidden child has completed login.
    #[cfg(target_os = "windows")]
    pub clipboard_fast_connected: Arc<AtomicBool>,
    /// Avoid flooding the log once per CLIPRDR range request while falling back.
    #[cfg(target_os = "windows")]
    pub clipboard_fast_fallback_logged: Arc<AtomicBool>,
    /// Parent id currently owned by the child kept in clipboard_fast_thread.
    #[cfg(target_os = "windows")]
    pub clipboard_fast_active_parent_conn_id: Arc<AtomicI32>,
    /// Most recent visible connection that should own the next fast child.
    #[cfg(target_os = "windows")]
    pub clipboard_fast_desired_parent_conn_id: Arc<AtomicI32>,
    /// Ensures only one bounded restart waiter is created for a stale child.
    #[cfg(target_os = "windows")]
    pub clipboard_fast_restart_pending: Arc<AtomicBool>,
    /// Keeps the hidden child's thread alive for the lifetime of the parent.
    #[cfg(target_os = "windows")]
    pub clipboard_fast_thread: Arc<Mutex<Option<std::thread::JoinHandle<()>>>>,
}
...
impl<T: InvokeUiSession> Session<T> {
    ...
    #[inline]
    pub fn is_clipboard_fast(&self) -> bool {
        #[cfg(target_os = "windows")]
        {
            self.clipboard_fast
        }
    ...
    #[cfg(target_os = "windows")]
    pub fn start_clipboard_fast_channel(&self, parent_conn_id: i32) -> bool {
        if !self.is_default() || parent_conn_id <= 0 {
            self.log_clipboard_fast_fallback("invalid-parent");
            return false;
        }

        // A bounded restart waiter may already be draining an older child. Keep
        // its target current across any number of rapid visible reconnects.
        self.clipboard_fast_desired_parent_conn_id
            .store(parent_conn_id, Ordering::Release);

        let Some(conn_token) = self.get_conn_token() else {
            self.log_clipboard_fast_fallback("missing-conn-token");
            return false;
        };

        let mut thread = self.clipboard_fast_thread.lock().unwrap();
        if thread
            .as_ref()
            .is_some_and(|running| !running.is_finished())
        {
            let active_parent = self
                .clipboard_fast_active_parent_conn_id
                .load(Ordering::Acquire);
            if active_parent == parent_conn_id {
                return true;
            }

            if let Some(sender) = self.clipboard_fast_sender.read().unwrap().as_ref() {
                sender.send(Data::Close).ok();
            }
            if self
                .clipboard_fast_restart_pending
                ...
                .is_ok()
            {
                ...
                std::thread::spawn(move || {
                    ...
                    for _ in 0..ATTEMPTS {
                        if let Some(sender) = parent.clipboard_fast_sender.read().unwrap().as_ref()
                        {
                            sender.send(Data::Close).ok();
                        }
                        let finished = parent
                            .clipboard_fast_thread
                            .lock()
                            .unwrap()
                            .as_ref()
                        ...
                        if finished {
                            ...
                            let desired_parent = parent
                                .clipboard_fast_desired_parent_conn_id
                                .load(Ordering::Acquire);
                            parent.start_clipboard_fast_channel(desired_parent);
                            return;
                        }
                    ...
                    parent
                        .clipboard_fast_restart_pending
                        .store(false, Ordering::Release);
                    parent.log_clipboard_fast_fallback("restart-timeout");
                });
            }
            return true;
        }
        ...
        self.clipboard_fast_connected
            .store(false, Ordering::Release);
        *self.clipboard_fast_sender.write().unwrap() = None;
        self.clipboard_fast_active_parent_conn_id
            .store(parent_conn_id, Ordering::Release);

        let (id, force_relay, adapter_luid) = {
            let lc = self.lc.read().unwrap();
            (lc.get_id().to_owned(), lc.force_relay, lc.adapter_luid)
        };
        let child = Session::<T> {
            ui_handler: T::default(),
            server_keyboard_enabled: self.server_keyboard_enabled.clone(),
            server_file_transfer_enabled: self.server_file_transfer_enabled.clone(),
            server_clipboard_enabled: self.server_clipboard_enabled.clone(),
            clipboard_fast: true,
            clipboard_parent_conn_id: parent_conn_id,
            clipboard_fast_connected: self.clipboard_fast_connected.clone(),
            clipboard_fast_fallback_logged: self.clipboard_fast_fallback_logged.clone(),
            clipboard_fast_sender: self.clipboard_fast_sender.clone(),
            clipboard_fast_active_parent_conn_id: self.clipboard_fast_active_parent_conn_id.clone(),
            clipboard_fast_desired_parent_conn_id: self
                .clipboard_fast_desired_parent_conn_id
                .clone(),
            clipboard_fast_restart_pending: self.clipboard_fast_restart_pending.clone(),
            ..Default::default()
        };
        {
            let mut lc = child.lc.write().unwrap();
            lc.initialize(
                id.clone(),
            ...
            lc.clipboard_fast = true;
        }

        match std::thread::Builder::new()
            .name("clipboard-fast-channel".to_owned())
            .spawn(move || {
                let round = child.connection_round_state.lock().unwrap().new_round();
                io_loop(child, round);
            }) {
            Ok(handle) => {
                *thread = Some(handle);
                log::info!(
                    "clipboard-fast-channel=connecting peer_id={} parent_conn_id={}",
                    id,
                    parent_conn_id
                );
                true
            ...
            Err(err) => {
                log::warn!(
                    "clipboard-fast-channel=fallback reason=spawn-failed peer_id={} error={}",
                    id,
                    err
                );
                self.clipboard_fast_fallback_logged
    ...
    #[cfg(target_os = "windows")]
    pub fn send_clipboard_fast(&self, message: Message) -> bool {
        if !self.clipboard_fast_connected.load(Ordering::Acquire) {
            return false;
        }
        let Some(child_sender) = self.clipboard_fast_sender.read().unwrap().clone() else {
            self.clipboard_fast_connected
                .store(false, Ordering::Release);
            return false;
        };
        let Some(main_sender) = self.sender.read().unwrap().clone() else {
            return false;
        };
        let (ack_sender, mut ack_receiver) = mpsc::unbounded_channel();
        if child_sender
            .send(Data::ClipboardFastMessage((message.clone(), ack_sender)))
            .is_err()
        {
            self.clipboard_fast_connected
                .store(false, Ordering::Release);
        ...
        let connected = self.clipboard_fast_connected.clone();
        let peer_id = self.get_id();
        tokio::spawn(async move {
            if ack_receiver.recv().await != Some(true) {
                connected.store(false, Ordering::Release);
                log::warn!(
                    "clipboard-fast-channel=fallback reason=child-queue-dropped peer_id={}",
                    peer_id
                );
                // The child never attempted the network write, so replay is
                // safe. If a write was attempted (even if it returned an
                // error), the ack is true and disconnect wakes the IStream;
                // replay could otherwise duplicate a partially written frame.
                main_sender.send(Data::Message(message)).ok();
            }
        });
        true
    }
    ...
    #[cfg(target_os = "windows")]
    pub fn stop_clipboard_fast_channel(&self) {
        if let Some(sender) = self.clipboard_fast_sender.read().unwrap().as_ref() {
            sender.send(Data::Close).ok();
        }
    }
    ...
    #[cfg(target_os = "windows")]
    pub fn log_clipboard_fast_fallback(&self, reason: &str) {
        if !self
            .clipboard_fast_fallback_logged
            .swap(true, Ordering::AcqRel)
        {
            log::info!(
                "clipboard-fast-channel=fallback reason={} peer_id={} parent_conn_id={}",
                reason,
                self.get_id(),
                self.clipboard_fast_active_parent_conn_id
                    .load(Ordering::Acquire)
...
impl<T: InvokeUiSession> Interface for Session<T> {
    ...
    fn msgbox(&self, msgtype: &str, title: &str, text: &str, link: &str) {
        ...
        if self.clipboard_fast {
            log::debug!(
                "suppress hidden clipboard-fast msgbox type={} title={} text={}",
                msgtype,
                title,
                text
            );
    ...
    fn handle_login_error(&self, err: &str) -> bool {
        ...
        if self.clipboard_fast {
            log::warn!(
                "clipboard-fast-channel=fallback reason=login-error peer_id={} error={}",
                self.get_id(),
                err
            );
            self.clipboard_fast_fallback_logged
    ...
    fn handle_peer_info(&self, mut pi: PeerInfo) {
        ...
        #[cfg(target_os = "windows")]
        if self.clipboard_fast {
            // The hidden child exists only as a transport. Do not publish it as
            // a file-manager session or open/close any UI.
...
pub async fn io_loop<T: InvokeUiSession>(handler: Session<T>, round: u32) {
    ...
    *handler.sender.write().unwrap() = Some(sender.clone());
    #[cfg(target_os = "windows")]
    if handler.clipboard_fast {
        *handler.clipboard_fast_sender.write().unwrap() = Some(sender.clone());
    }
