...
pub fn test_nat_type() {
    ...
    std::thread::spawn(move || {
        ...
        // Plan A: the NAT test runs on the official UDP path (21115) in both
        // modes; VLESS only wraps the relay transport.
        let is_direct = Config::get_socks().is_none() && !config::use_ws();
        if !is_direct {
...
fn get_api_server_(api: String, custom: String) -> String {
    ...
    // Custom build: a basic hbbs has no api endpoint, so do not derive the
    // 21114 fallback from the rendezvous host; all api callers skip on empty.
    let _ = custom;
    "".to_owned()
}
