...
use hbb_common::{
    ...
    config::{self, keys::OPTION_ALLOW_WEBSOCKET, Config, Config2, SshServer, VlessServer},
...
pub enum Data {
    ...
    #[cfg(target_os = "windows")]
    ClipboardFastChannel(bool),
    #[cfg(target_os = "windows")]
    FastClipboard(ClipboardFile),
...
async fn handle(data: Data, stream: &mut Connection) {
    match data {
        ...
        Data::Vless(v) => match v {
            None => {
                allow_err!(stream.send(&Data::Vless(Config::get_vless_config())).await);
            }
            Some(data) => {
                let _nat = CheckTestNatType::new();
                Config::set_vless(Some(data));
                RendezvousMediator::restart();
                log::info!("vless updated");
            }
        },
        Data::Ssh(s) => match s {
            None => {
                allow_err!(stream.send(&Data::Ssh(Config::get_ssh_config())).await);
            }
            Some(data) => {
                let _nat = CheckTestNatType::new();
                Config::set_ssh(Some(data));
                RendezvousMediator::restart();
                log::info!("ssh updated");
            }
        },
...
#[inline]
async fn get_vless_(ms_timeout: u64) -> ResultType<Option<VlessServer>> {
    let mut c = connect(ms_timeout, "").await?;
    c.send(&Data::Vless(None)).await?;
    if let Some(Data::Vless(value)) = c.next_timeout(ms_timeout).await? {
        Config::set_vless(value.clone());
        Ok(value)
    } else {
        Ok(Config::get_vless_config())
    }
}

pub async fn get_vless_async(ms_timeout: u64) -> Option<VlessServer> {
    get_vless_(ms_timeout)
        .await
        .unwrap_or(Config::get_vless_config())
}

#[tokio::main(flavor = "current_thread")]
pub async fn get_vless() -> Option<VlessServer> {
    get_vless_async(1_000).await
}

#[tokio::main(flavor = "current_thread")]
pub async fn set_vless(value: VlessServer) -> ResultType<()> {
    let _nat = CheckTestNatType::new();
    Config::set_vless(Some(value.clone()));
    connect(1_000, "")
        .await?
        .send(&Data::Vless(Some(value)))
        .await?;
    Ok(())
}

#[inline]
async fn get_ssh_(ms_timeout: u64) -> ResultType<Option<SshServer>> {
    let mut c = connect(ms_timeout, "").await?;
    c.send(&Data::Ssh(None)).await?;
    if let Some(Data::Ssh(value)) = c.next_timeout(ms_timeout).await? {
        Config::set_ssh(value.clone());
        Ok(value)
    } else {
        Ok(Config::get_ssh_config())
    }
}

pub async fn get_ssh_async(ms_timeout: u64) -> Option<SshServer> {
    get_ssh_(ms_timeout)
        .await
        .unwrap_or(Config::get_ssh_config())
}

#[tokio::main(flavor = "current_thread")]
pub async fn get_ssh() -> Option<SshServer> {
    get_ssh_async(1_000).await
}

#[tokio::main(flavor = "current_thread")]
pub async fn set_ssh(value: SshServer) -> ResultType<()> {
    let _nat = CheckTestNatType::new();
    Config::set_ssh(Some(value.clone()));
    connect(1_000, "")
        .await?
        .send(&Data::Ssh(Some(value)))
        .await?;
    Ok(())
}
