...
impl<T: InvokeUiSession> Remote<T> {
    ...
    pub async fn io_loop(&mut self, key: &str, token: &str, round: u32) {
        ...
        .await
        {
            Ok(((mut peer, direct, pk, kcp, stream_type), (feedback, rendezvous_server))) => {
                ...
                if !self.handler.is_clipboard_fast() {
                    self.handler
    ...
    fn handle_disconnected(&self, round: u32) {
        ...
        #[cfg(target_os = "windows")]
        if self.handler.clipboard_fast {
            let was_connected = self
                .handler
                .clipboard_fast_connected
                .swap(false, Ordering::AcqRel);
            *self.handler.clipboard_fast_sender.write().unwrap() = None;
            self.handler
                .clipboard_fast_active_parent_conn_id
                .store(0, Ordering::Release);
            let parent_conn_id = self.handler.clipboard_parent_conn_id;
            if let Err(err) = ContextSend::proc(|context| {
                context
                    .set_fast_channel(parent_conn_id, false)
                    .map_err(|e| e.into())
            }) {
                log::warn!(
                    "clipboard-fast-channel=disconnected peer_id={} parent_conn_id={} clear_error={}",
                    self.handler.get_id(),
                    parent_conn_id,
                    err
                );
                ...
                log::info!(
                    "clipboard-fast-channel=disconnected peer_id={} parent_conn_id={}",
                    self.handler.get_id(),
                    parent_conn_id
                );
            }
        ...
        if self.handler.is_default() && _set_disconnected_ok {
            self.handler.stop_clipboard_fast_channel();
        }
            ...
            Some(clip) => match clip {
                ...
                _ => {
                    ...
                    if stop {
                        ...
                        #[cfg(target_os = "windows")]
                        if use_fast_channel {
                            if self.handler.send_clipboard_fast(msg.clone()) {
                                log::trace!("routed clipboard file contents over fast channel");
                                return;
                            }
                            if self.peer_info.clipboard_fast_channel {
                                // This request uses the compatibility path, but
                                // opportunistically restore the child for the
                                // next CLIPRDR range after a transient drop.
                                self.handler
                                    .start_clipboard_fast_channel(self.client_conn_id);
                            }
                            self.handler
                                .log_clipboard_fast_fallback("channel-not-connected");
                        }
    ...
    async fn handle_msg_from_ui(&mut self, data: Data, peer: &mut Stream) -> bool {
        match data {
            ...
            #[cfg(target_os = "windows")]
            Data::ClipboardFastMessage((msg, ack)) => {
                // `true` commits this dequeued frame to the network-write
                // path; it is not a remote acknowledgement. From this point,
                // replay on the parent could duplicate a partially written
                // stream id. A failed write closes the child instead.
                ack.send(true).ok();
                let result = peer.send(&msg).await;
                if let Err(err) = result {
                    log::warn!(
                        "clipboard-fast-channel=disconnected reason=network-send peer_id={} error={}",
                        self.handler.get_id(),
                        err
                    );
                    return false;
            ...
            match msg_in.union {
                ...
                Some(message::Union::LoginResponse(lr)) => match lr.union {
                    ...
                    Some(login_response::Union::PeerInfo(pi)) => {
                        ...
                        #[cfg(target_os = "windows")]
                        if self.handler.clipboard_fast && !self.activate_clipboard_fast_channel() {
                            // Removing the child mapping on the peer is what
                            // makes its main connection resume compatibility
                            // routing; do not leave a half-active fast child.
                            return false;
                        }
                        #[cfg(all(target_os = "windows", not(feature = "flutter")))]
                        self.check_clipboard_file_context();
                        if self.handler.is_default() {
                            ...
                            if self.peer_info.clipboard_fast_channel {
                                ...
                                self.handler.log_clipboard_fast_fallback("peer-unsupported");
                            }
                            #[cfg(feature = "flutter")]
                            #[cfg(not(target_os = "ios"))]
                        ...
                        if self.handler.is_file_transfer() && !self.handler.is_clipboard_fast() {
    ...
    fn set_peer_info(&mut self, pi: &PeerInfo) {
            ...
            serde_json::from_str::<HashMap<String, serde_json::Value>>(&pi.platform_additions)
        {
            ...
            self.peer_info.support_view_camera = platform_additions
                ...
                .flatten()
                .unwrap_or(false);
            self.peer_info.clipboard_fast_channel = platform_additions
                .get("clipboard_fast_channel")
    ...
    #[cfg(target_os = "windows")]
    fn activate_clipboard_fast_channel(&self) -> bool {
        let parent_conn_id = self.handler.clipboard_parent_conn_id;
        let result = ContextSend::make_sure_enabled().and_then(|_| {
            ContextSend::proc(|context| {
                context
                    .set_fast_channel(parent_conn_id, true)
                    .map_err(|e| e.into())
            })
        });
        match result {
            Ok(()) => {
                ...
                if first {
                    log::info!(
                        "clipboard-fast-channel=connected peer_id={} parent_conn_id={}",
                        self.handler.get_id(),
                        parent_conn_id
                    );
                }
            ...
            Err(err) => {
                ...
                log::warn!(
                    "clipboard-fast-channel=fallback reason=context-setup peer_id={} parent_conn_id={} error={}",
                    self.handler.get_id(),
                    parent_conn_id,
                    err
                );
    ...
    async fn handle_cliprdr_msg(
        ...
        if !self.handler.is_clipboard_fast()
            && matches!(
                &clip.union,
                Some(hbb_common::message_proto::cliprdr::Union::FormatList(_))
            )
        {
        ...
        if !stop {
            ...
            #[cfg(target_os = "windows")]
            {
                let context_conn_id = if self.handler.clipboard_fast {
                    self.handler.clipboard_parent_conn_id
                } else {
                    self.client_conn_id
                };
