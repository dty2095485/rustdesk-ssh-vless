...
pub struct RendezvousServer {
    ...
    tcp_senders: Arc<Mutex<HashMap<SocketAddr, mpsc::UnboundedSender<Bytes>>>>,
    pm: PeerMap,
    tx: Sender,
...
impl RendezvousServer {
    ...
    pub async fn start_with_bind(
        ...
        let vless_hbbs_addr = parse_vless_internal_addr("VLESS_HBBS_INTERNAL")?;
        let vless_nat_addr = parse_vless_internal_addr("VLESS_NAT_INTERNAL")?;
        let mut vless_hbbs_listener = create_vless_internal_listener(vless_hbbs_addr).await?;
        let mut vless_nat_listener = create_vless_internal_listener(vless_nat_addr).await?;
            ...
            loop {
                ...
                match rs
                    .io_loop(
                        ...
                        &mut vless_hbbs_listener,
                        &mut vless_nat_listener,
                        &mut socket,
                        &key,
                    )
                    .await
                {
                    ...
                    LoopFailure::VlessHbbsListener => {
                        drop(vless_hbbs_listener.take());
                        vless_hbbs_listener =
                            create_vless_internal_listener(vless_hbbs_addr).await?;
                    }
                    LoopFailure::VlessNatListener => {
                        drop(vless_nat_listener.take());
                        vless_nat_listener = create_vless_internal_listener(vless_nat_addr).await?;
                    }
                }
            }
        };
    ...
    async fn io_loop(
        ...
        vless_hbbs_listener: &mut Option<TcpListener>,
        vless_nat_listener: &mut Option<TcpListener>,
            ...
            tokio::select! {
                ...
                Some(data) = rx.recv() => {
                    match data {
                        Data::Msg(msg, addr) => {
                            ...
                            if let Some(tx) = self.tcp_senders.lock().await.get(&key).cloned() {
                                // Deliver over the peer's persistent TCP connection.
                ...
                res = listener2.accept() => {
                    match res {
                        Ok((stream, addr))  => {
                            ...
                            self.handle_listener2(stream, addr, false).await;
                        }
                        Err(err) => {
                ...
                res = listener3.accept() => {
                    match res {
                        Ok((stream, addr))  => {
                            stream.set_nodelay(true).ok();
                            self.handle_listener(stream, addr, key, true, false).await;
                        }
                        Err(err) => {
                ...
                res = listener.accept() => {
                    match res {
                        Ok((stream, addr)) => {
                            stream.set_nodelay(true).ok();
                            self.handle_listener(stream, addr, key, false, false).await;
                        }
                       Err(err) => {
                ...
                res = accept_or_pending(vless_hbbs_listener.as_ref()) => {
                    match res {
                        Ok((stream, addr)) => {
                            stream.set_nodelay(true).ok();
                            self.handle_listener(stream, addr, key, false, true).await;
                        }
                        Err(err) => {
                            log::error!("VLESS hbbs internal listener.accept failed: {}", err);
                            return LoopFailure::VlessHbbsListener;
                        }
                    }
                }
                res = accept_or_pending(vless_nat_listener.as_ref()) => {
                    match res {
                        Ok((stream, addr)) => {
                            stream.set_nodelay(true).ok();
                            self.handle_listener2(stream, addr, true).await;
                        }
                        Err(err) => {
                            log::error!("VLESS NAT internal listener.accept failed: {}", err);
                            return LoopFailure::VlessNatListener;
                        }
                    }
                }
            }
        ...
        if let Ok(msg_in) = RendezvousMessage::parse_from_bytes(bytes) {
            match msg_in.union {
                Some(rendezvous_message::Union::PunchHoleRequest(ph)) => {
                    ...
                    allow_err!(
                        self.handle_tcp_punch_hole_request(addr, ph, key, ws, vless_authenticated,)
                            .await
                    );
                    return true;
                }
                ...
                Some(rendezvous_message::Union::RelayResponse(mut rr)) => {
                    ...
                    if !rr.relay_server.is_empty() {
                        let target_is_vless = self
                            .tcp_senders
                            .lock()
                            .await
                            .contains_key(&try_into_v4(addr_b));
                        // A VLESS peer uses a loopback transport endpoint, but
                        // it is not part of the server's LAN.  Use the explicit
                        // authenticated-session registry instead of inferring
                        // transport type from the source address.
                        if self.is_lan(addr_b) && !target_is_vless {
                ...
                Some(rendezvous_message::Union::RegisterPeer(rp)) => {
                    // Only the dedicated loopback listener can authenticate a
                    // TCP-only VLESS registration. Source IP ranges are not an
                    // authentication signal: official connections keep the
                    // upstream behavior even when they originate on this host
                    // or a private Docker network.
                    if !vless_authenticated {
                        return false;
                    }
                    // Refresh in-memory registration so TCP-registered clients
                    // (e.g. behind VLESS tunnels) stay "online" for other peers.
                    let mut request_pk = true;
                    if !rp.id.is_empty() {
                        if let Some(old) = self.pm.get_in_memory(&rp.id).await {
                            let mut old = old.write().await;
                            // The loopback endpoint belongs to the authenticated
                            // gateway, not to the remote client.  Do not treat a
                            // gateway socket change as a client IP change.
                            request_pk = old.pk.is_empty();
                            if !request_pk {
                                old.socket_addr = addr;
                ...
                Some(rendezvous_message::Union::RegisterPk(rk)) => {
                    if vless_authenticated {
                        // TCP-only tunnels (e.g. VLESS) register through this
                        // connection.
                        let res = self.handle_tcp_register_pk(rk, addr).await;
                        let mut msg_out = RendezvousMessage::new();
                        msg_out.set_register_pk_response(RegisterPkResponse {
                            result: res.into(),
                            ..Default::default()
                    ...
                    // Stock behavior for official listeners: TCP RegisterPk is
                    // unsupported, so clients continue registering over UDP.
                    let mut msg_out = RendezvousMessage::new();
                    msg_out.set_register_pk_response(RegisterPkResponse {
    ...
    // Mirrors the UDP RegisterPk flow for identity validation and persistence.
    // The shared loopback gateway address deliberately skips the global
    // per-source-IP limiter; the per-peer RegisterPk limiter below remains.
    async fn handle_tcp_register_pk(
        &mut self,
        rk: RegisterPk,
        addr: SocketAddr,
        ...
        let (changed, ip_changed) = {
            ...
            if peer.uuid.is_empty() {
                ...
                let ip_changed = peer.info.ip != ip;
                (peer.pk != rk.pk || ip_changed, ip_changed)
            } else {
                log::warn!(
                    "Peer {} uuid mismatch: {:?} vs {:?}",
                    id,
    ...
    async fn handle_punch_hole_request(
        ...
        if let Some(peer) = self.pm.get(&id).await {
            ...
            let mut msg_out = RendezvousMessage::new();
            let peer_is_vless = self
                .tcp_senders
                .lock()
                .await
                .contains_key(&try_into_v4(peer_addr));
            let peer_is_lan = !peer_is_vless && self.is_lan(peer_addr);
            let is_lan = !vless_authenticated && self.is_lan(addr);
            ...
            let same_intranet: bool = !vless_authenticated
                && !peer_is_vless
                && !ws
                && (peer_is_lan && is_lan || {
    ...
    async fn handle_tcp_punch_hole_request(
        ...
        let (msg, to_addr) = self
            .handle_punch_hole_request(addr, ph, key, ws, vless_authenticated)
            .await?;
    ...
    async fn handle_udp_punch_hole_request(
        ...
        let (msg, to_addr) = self
            .handle_punch_hole_request(addr, ph, key, false, false)
            .await?;
    ...
    async fn handle_listener2(
        ...
        if ip.is_loopback() && !vless_authenticated {
            tokio::spawn(async move {
    ...
    async fn handle_listener(
        ...
        log::debug!(
            "Tcp connection from {:?}, ws: {}, vless_authenticated: {}",
            addr,
            ws,
            vless_authenticated
        );
        ...
        tokio::spawn(async move {
            allow_err!(
                rs.handle_listener_inner(stream, addr, &key, ws, vless_authenticated)
                    .await
            );
        });
    }
    ...
    async fn handle_listener_inner(
        ...
        if ws {
            ...
            while let Ok(Some(Ok(msg))) = timeout(30_000, b.next()).await {
                if let tungstenite::Message::Binary(bytes) = msg {
                    if !self
                        .handle_tcp(&bytes, &mut sink, addr, key, ws, vless_authenticated)
                        .await
                    {
                        break;
                    }
        ...
        } else if vless_authenticated {
            let (a, mut b) = Framed::new(stream, BytesCodec::new()).split();
            sink = Some(Sink::TcpStream(a));
            // Persistent server->client channel so the server can push
            // punch/relay instructions to this connection at any time.
            let (tx, mut rx) = mpsc::unbounded_channel::<Bytes>();
            self.tcp_senders.lock().await.insert(try_into_v4(addr), tx);
            // Send empty heartbeats so long-lived TCP connections (e.g. clients
            // registering through VLESS tunnels) are not dropped when idle.
                ...
                tokio::select! {
                    n = b.next() => {
                        match n {
                            Some(Ok(bytes)) => {
                                if bytes.is_empty() {
                                    continue; // client heartbeat reply
                                }
                                if !self
                                    .handle_tcp(
                                        &bytes,
                                    ...
                                    .await
                                {
                                    log::debug!("tcp loop break: handle_tcp returned false for {} bytes", bytes.len());
                                    break;
                                }
                            }
                            other => {
                                log::debug!("tcp loop break: next() = {:?}", other.as_ref().map(|x| x.as_ref().map(|b| b.len())));
                                break;
                            }
                        }
                    }
                    ...
                    _ = heartbeat.tick() => {
                        // Punch/relay requests temporarily move the write half
                        // into tcp_punch.  Keep reading until the peer closes;
                        // otherwise a slow response races the heartbeat and is
                        // discarded as an apparent disconnect.
                        if sink.is_some() {
                            Self::send_to_sink(&mut sink, RendezvousMessage::default()).await;
                        }
                    }
                }
            }
            self.tcp_senders.lock().await.remove(&try_into_v4(addr));
        } else {
            // Direct clients (including iOS) retain the upstream TCP/UDP
            // rendezvous behavior. The persistent sender is VLESS-only.
            let (a, mut b) = Framed::new(stream, BytesCodec::new()).split();
            sink = Some(Sink::TcpStream(a));
...
fn parse_vless_internal_addr(name: &str) -> ResultType<Option<SocketAddr>> {
    let value = std::env::var(name).unwrap_or_default();
    parse_vless_internal_addr_value(name, &value)
}

fn parse_vless_internal_addr_value(name: &str, value: &str) -> ResultType<Option<SocketAddr>> {
    let value = value.trim();
    if value.is_empty() {
        return Ok(None);
    }
    let addr: SocketAddr = match value.parse() {
        Ok(addr) => addr,
        Err(err) => bail!("Invalid {name} address {value:?}: {err}"),
    };
    if !addr.ip().is_loopback() {
        bail!("{name} must bind to a loopback address, got {addr}");
    }
    Ok(Some(addr))
}

async fn create_vless_internal_listener(
    addr: Option<SocketAddr>,
) -> ResultType<Option<TcpListener>> {
    let Some(addr) = addr else {
        return Ok(None);
    };
    let listener = hbb_common::tcp::new_listener(addr, true).await?;
    log::info!("Listening on tcp {} for authenticated VLESS", addr);
    Ok(Some(listener))
}
...
mod tests {
    ...
    #[test]
    fn vless_internal_addr_is_optional_and_loopback_only() {
        assert!(parse_vless_internal_addr_value("TEST", "")
            .unwrap()
            .is_none());
        assert_eq!(
            parse_vless_internal_addr_value("TEST", "127.0.0.1:22116").unwrap(),
            Some("127.0.0.1:22116".parse().unwrap())
        );
        assert_eq!(
            parse_vless_internal_addr_value("TEST", "[::1]:22116").unwrap(),
            Some("[::1]:22116".parse().unwrap())
        );
        assert!(parse_vless_internal_addr_value("TEST", "0.0.0.0:22116").is_err());
        assert!(parse_vless_internal_addr_value("TEST", "172.18.0.2:22116").is_err());
        assert!(parse_vless_internal_addr_value("TEST", "not-an-address").is_err());
    }
}
