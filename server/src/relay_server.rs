...
const VLESS_HBBR_INTERNAL_ENV: &str = "VLESS_HBBR_INTERNAL";

fn parse_vless_internal_addr(
    value: Option<&str>,
    ...
    let addr = value
        .parse::<SocketAddr>()
        .map_err(|err| format!("Invalid {VLESS_HBBR_INTERNAL_ENV} address '{value}': {err}"))?;
    if !addr.ip().is_loopback() {
        return Err(format!(
            "{VLESS_HBBR_INTERNAL_ENV} must bind to a loopback address, got {addr}"
        ));
    }
    Ok(Some(addr))
}

fn relay_key_is_valid(key: &str, licence_key: &str, vless_authenticated: bool) -> bool {
    key.is_empty() || licence_key == key || vless_authenticated
}

#[tokio::main(flavor = "multi_thread")]
pub async fn start_with_bind(bind_addr: Option<IpAddr>, port: &str, key: &str) -> ResultType<()> {
    let key = get_server_sk(key);
    let vless_internal_value = std::env::var(VLESS_HBBR_INTERNAL_ENV).ok();
    let vless_internal_addr = match parse_vless_internal_addr(vless_internal_value.as_deref()) {
        Ok(addr) => addr,
        Err(err) => bail!(err),
    };
    if let Some(addr) = vless_internal_addr {
        log::info!(
            "VLESS-authenticated HBBr internal endpoint configured on {}",
            addr
        );
    } else {
        log::info!(
            "VLESS-authenticated HBBr internal endpoint disabled; set {} to a loopback socket address to enable it",
            VLESS_HBBR_INTERNAL_ENV
        );
    }
    ...
    let main_task = async move {
        loop {
            log::info!("Start");
            let listener_vless = if let Some(addr) = vless_internal_addr {
                let listener = hbb_common::tcp::new_listener(addr, true).await?;
                log::info!(
                    "Listening on tcp {} for authenticated VLESS relay traffic",
                    listener.local_addr()?
                );
                Some(listener)
            } else {
...
async fn io_loop(
    ...
    listener_vless: Option<TcpListener>,
    key: &str,
) {
    check_params();
        ...
        tokio::select! {
            ...
            res = listener2.accept() => {
                match res {
                    Ok((stream, addr))  => {
                        stream.set_nodelay(true).ok();
                        handle_connection(stream, addr, &limiter, key, true, false).await;
                    }
                    Err(err) => {
            ...
            res = crate::common::accept_or_pending(listener_vless.as_ref()) => {
                match res {
                    Ok((stream, addr)) => {
                        stream.set_nodelay(true).ok();
                        handle_connection(stream, addr, &limiter, key, false, true).await;
                    }
                    Err(err) => {
                       log::error!("VLESS internal listener.accept failed: {}", err);
                       break;
                    }
                }
            }
...
async fn handle_connection(
    ...
    if vless_authenticated && !ip.is_loopback() {
        log::error!(
            "Rejected authenticated VLESS relay connection from non-loopback peer {}",
            addr
        );
        return;
    }
    if !ws && !vless_authenticated && ip.is_loopback() {
    ...
    let limiter = limiter.clone();
    tokio::spawn(async move {
        allow_err!(make_pair(stream, addr, &key, limiter, ws, vless_authenticated).await);
    });
}
...
async fn make_pair(
    ...
    if ws {
        ...
        make_pair_(ws_stream, addr, key, limiter, vless_authenticated).await;
    } else {
        make_pair_(
    ...
    if let Ok(Some(Ok(bytes))) = timeout(30_000, stream.recv()).await {
        if let Ok(msg_in) = RendezvousMessage::parse_from_bytes(&bytes) {
            if let Some(rendezvous_message::Union::RequestRelay(rf)) = msg_in.union {
                if !relay_key_is_valid(key, &rf.licence_key, vless_authenticated) {
...
mod tests {
    ...
    #[test]
    fn vless_internal_listener_accepts_only_loopback_addresses() {
        assert_eq!(parse_vless_internal_addr(None).unwrap(), None);
        assert_eq!(parse_vless_internal_addr(Some("  ")).unwrap(), None);
        assert_eq!(
            parse_vless_internal_addr(Some("127.0.0.1:22117")).unwrap(),
            Some("127.0.0.1:22117".parse().unwrap())
        );
        assert_eq!(
            parse_vless_internal_addr(Some("[::1]:22117")).unwrap(),
            Some("[::1]:22117".parse().unwrap())
        );
        assert!(parse_vless_internal_addr(Some("0.0.0.0:22117")).is_err());
        assert!(parse_vless_internal_addr(Some("192.0.2.1:22117")).is_err());
        assert!(parse_vless_internal_addr(Some("not-a-socket")).is_err());
    }

    #[test]
    fn only_authenticated_vless_listener_can_bypass_inner_relay_key() {
        assert!(relay_key_is_valid("", "anything", false));
        assert!(relay_key_is_valid("server-key", "server-key", false));
        assert!(!relay_key_is_valid("server-key", "fallback-key", false));
        assert!(relay_key_is_valid("server-key", "fallback-key", true));
    }
}
